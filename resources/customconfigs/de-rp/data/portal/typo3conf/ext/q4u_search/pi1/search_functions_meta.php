<?php
function logit($text){
	 	if($h = fopen("/tmp/q4usearch_log.txt","a")){
			...
			foreach($interfaces as $key => $xml_cat) {
				...
				if(!file_exists($filename) || filesize($filename)==0 && $currentpage!=0 && !$in_ajax) {
					...
					if($currentpage<$npages) {
						file_get_contents('http://localhost/mapbender/geoportal/mod_readCSWResults.php?q='.$query.'&p='.($currentpage+1).'&cat='.$key.'&request_id='.$unique);
					}
				}

				if(file_exists($filename) && filesize($filename)>0) {
					//logit("found! : ".$filename);
					try {
						...
						if($xmlData) {
							...
							foreach($xmlData->result as $result) {
					       	...
					       	if($result->wmsaccessUrl!='') {
										$out['temp'].='
											<div class="search-mapicons">
												<a href="'.$L['KarteURL'].'?WMS='.$result->wmsaccessUrl.'"><img src="fileadmin/design/icn_map.png" title="Auf Karte anzeigen" /></a>
											</div>';
						...
						if($result->accessUrl!='') {
									$out['temp'].='<a onclick="openwindow(this.href); return false" href="'.urldecode($result->accessUrl).'"><img height="15" width="15" src="fileadmin/design/url.png" title="" /></a>';
						}
									$out['temp'].='		</div>';
						if($graphicUrl!='') {
									$out['temp'].='	
											<div class="search-metaimage">
												<img src="'.urldecode($graphicUrl).'" />
											</div>';
						}
									$out['temp'].='	
					...
					#       	$out['temp'].='<div class="search-metacat">'.urldecode($result->categorie).'</div>\n';
						...
						/*if($result->accessUrl!='') {
						       	$out['temp'].='
						       		<div class="search-metacat">
						       			<a target="_blank" href="'.urldecode($result->accessUrl).'" onclick="openwindow(this.href); return false">Weitere Informationen</a>
						       		</div>';
					       	}*/
