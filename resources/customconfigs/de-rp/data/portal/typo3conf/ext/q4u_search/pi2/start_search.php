...
//$e = new mb_exception("pi2/start_search.php invoked!!!");
...
$_REQUEST['uid']=($_REQUEST['uid']=='')?CreateUUID():preg_replace("/[^a-zA-Z0-9-]/", "",$_REQUEST['uid']);
...
$searchtext=$_REQUEST['searchText'].$_REQUEST['registratingDepartments'].$_REQUEST['isoCategories'].$_REQUEST['regTimeBegin'].$_REQUEST['regTimeEnd'].$_REQUEST['searchBbox'].$_REQUEST['searchTypeBbox'].$_REQUEST['searchResources'].$_REQUEST['timeBegin'].$_REQUEST['timeEnd'].$_REQUEST['restrictToOpenData'];
...
if(!$ajax) { // Normale Suche
	...
	$LinkURL=$L['ErwSuchURL'];
    	// BoundyBox übergeben -> Einschränkung in Session speichern
   	if(!empty($_REQUEST['searchBbox'])) {
        	$epsg=(!empty($_SESSION['epsg']))?array_pop(explode(':',$_SESSION['epsg'])):'31466';
        	convertCoordinates($_REQUEST['searchBbox'],'4326',$epsg);
    	}
...
// Metadaten
	...
	if($timer) {
		...
		<script type="text/javascript">
			...
			function AjaxSearch () {
				xajax_search("'.$unique.'","'.PtH($_REQUEST['searchid']).'", "'.PtH($cat).'", "'.PtH($page).'", "'.$_SESSION['mb_user_id'].'", "'.t3lib_div::GPvar('L').'", search_meta,search_dienste,search_filter);
			}
