...
// 		debug($selectConf,'select_conf');
	...
	function getItemMarkerArray ($row, $textRenderObj = 'displaySingle') {
		...
		if ($row['links']) {
			...
			$markerArray['###NEWS_LINKS###'] = $links_stdWrap[0].$newsLinks.$links_stdWrap[1];
		}

		// filelinks
		# q.ms001 öffnendes Tag wurde an ###TEXT_FILES### gehangen, Text ist nicht änderbar. Schließendes Tag wurde an ###FILE_LINK### gehangen -> großer Mist. Jetzt nur noch ###FILE_LINK###
		...
		if ($row['news_files']) {
			...
			$markerArray['###FILE_LINK###'] = $files_stdWrap[0].$filelinks.$files_stdWrap[1];
