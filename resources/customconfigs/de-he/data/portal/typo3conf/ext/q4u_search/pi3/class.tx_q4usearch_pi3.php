...
#		global $output;
		...
		if(file_exists($this->file) && filesize($this->file)>0) {
			...
			$output.='<a href="'.$this->url.'"><img src="fileadmin/design/icn_georss_22.png" alt="RSS" title="RSS" /></a>';
			$output.='<a href="'.$L['KarteURL'].'?GEORSS='.urlencode($this->url).'"><img src="fileadmin/design/icn_map_transp.png" alt="GeoRSS" title="GeoRSS" /></a>';
/*			$output.='<script src="//platform.twitter.com/widgets.js" type="text/javascript"></script><a href="https://twitter.com/GeoPortalRLP" class="twitter-follow-button" data-show-count="false" data-lang="'.$Lang.'">Follow @GeoPortalRLP</a><br>';
			$output.='<a href="https://twitter.com/share?url=http%3A%2F%2Fwww.geoportal.rlp.de" class="twitter-share-button" data-show-count="false" data-lang="'.$Lang.'"></a>';
*/
			try {
				...
				if($xmlObject) {
					...
					foreach($xmlObject->channel->item AS $item) {
						...
						$output.='
							...
							<h2><a title="'.$text.'" href="'.$item->link.'" onclick="window.open(this.href, \'Information\', \'width=500,height=600,left=100,top=100,scrollbars=yes,resizable=no\');return false;">'.$text.'</a></h2>
							'.$abstract.'
							</div>';
						$i++;
					}
