...
function find_page($pid) {
	...
	if($language!=0) {
		$sql="SELECT pages.uid, pages.pid, pages_language_overlay.title, pages.no_search, pages.doktype, tt_content.header, tt_content.subheader, tt_content.bodytext, tt_content.records, tt_content.CType
		       ...
		       WHERE pages.pid IN ($pid) AND pages.deleted=0 AND pages.hidden=0 AND $timestamp>=pages.starttime AND ($timestamp<=pages.endtime or pages.endtime=0) AND pages_language_overlay.sys_language_uid=".$db->v($language)." AND tt_content.sys_language_uid=".$db->v($language)."
...
// Nach Login zu durchsuchende Zweige
	   	...
	   	$ids[]=$uid;
			...
			if($crumb[$pid]=="") {
				$crumb[$uid]="<a href=\"".realurl_link(1,false,$language)."\">Geoportal</a> &gt; <a href='".realurl_link($uid,false,$language)."'>$title</a>";
			} else {
