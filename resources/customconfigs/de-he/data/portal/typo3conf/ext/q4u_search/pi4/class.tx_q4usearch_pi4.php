...
class tx_q4usearch_pi4 extends tslib_pibase {
	...
	function main($content,$conf)	{
		...
		//Bilder
		$url='http://localhost/mapbender/geoportal/mod_initialStartWmc.php?languageCode='.$Lang.'&outputFormat=json&maxObjects=12&hostName='.$_SERVER['HTTP_HOST'];
		if ($Lang == 'de') {
		...
		if(is_array($DATA->initialWmcDocs)) {
			foreach($DATA->initialWmcDocs as $tag) {
				$content.='
				<div class="wmcdocs">
					<a class="wmcdocimg" href="'.$tag->loadUrl.'" title="'.$tag->abstract.' ('.$tag->loadCount.')"><img src="'.$tag->previewUrl.'" width="155"/></a>';
					if ($tag->timestamp != 0) {
						$content.='<div class="newimage"><img src="/fileadmin/design/new_'.$Lang.'.png" alt="new"/></div>';
					}
					//for showing qr codes:
					if ($tag->mobileUrl != "" && $tag->mobileQrImageUrl != "") {
					$content.='<div class="qrcode">
						<a href="'.$tag->mobileUrl.'" target="_blank"><img src="'.$tag->mobileQrImageUrl.'" alt="Link to mobile version"/></a>
					</div>';
					}
					//showing metadata symbol (?)
