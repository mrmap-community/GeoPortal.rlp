...
function Adressen($file, $max=0) {
					...
					if(!$startpage) {
						if($counter>=($page*$items_page) && $counter<(($page+1)*$items_page)) {
							$out[$name].='
			      			...
			      			<div class="search-title all"><a href="karten.html?geomuid='.$_REQUEST['uid'].'&geomid='.$member->attributes()->id.'" alt="'.$Classes[$memberclass].'" title="'.$Classes[$memberclass].'">'.$member->FeatureCollection->featureMember->children()->children()->title.'</a></div>
								</div>
							';
						} else {
							++$counter;
	...
	$url=$L['SuchURL'].'?searchid='.$_REQUEST['searchid'].'&amp;selectsearch='.intval($_REQUEST['selectsearch']).'&amp;uid='.$_REQUEST['uid'].'&amp;adrcat='.$_REQUEST['adrcat'];
