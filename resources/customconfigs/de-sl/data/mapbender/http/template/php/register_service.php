...
<html lang="de" xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		...
		<title>geoportal.saarland.de | Metadaten</title>
	...
	<!-- 	<link rel="stylesheet" type="text/css" href="/fileadmin/design/geoportal.rlp.css" />   -->
		...
		<script type="text/javascript">
			function tou(tou) {
				fenster=window.open("php/termsofuse.php?tou="+tou, "TermsOfUse", "width=500,height=500,left=100,top=100,scrollbars=yes,resizable=no");
				fenster.focus();
				return false;
			}
		</script>
...
<body id="top" class="popup">
	...
	<a id="close" href="javascript:window.close()">Fenster schließen <img src="../img/search/icn_close.png" width="18" height="18" alt="" /></a>
		...
		<div id="center" class="content" style="padding:2em">
			...
			<h2><span><?php print "Antragsstellung Freischaltung"; ?></span></h2>

			<fieldset>
			<label class="text" for="name"><strong title="Pflichtfeld"><?php print "Benutzername"; ?></strong></label>
			<label class="text"><strong><?php print $DATA["Name"]; ?></strong></label>
			<br class="clr" />
			</fieldset>

			<fieldset>
			<label class="text" for="email"><strong title="Pflichtfeld"><?php print "EMail"; ?></strong></label>
			<label class="text"><strong><?php print $DATA["EMail"]; ?></strong></label>
			<br class="clr" />
			</fieldset>

			<fieldset>
			<label class="text" for="name"><strong title="Pflichtfeld"><?php print "Name"; ?></strong></label>
			<input class="text" id="realname" type="text" name="RealName" maxlength="40" value="" onfocus="clearField(this)" />
			<br class="clr" />
			</fieldset>

			<fieldset>
			<label class="text" for="telefon"><strong><?php print "Telefon"; ?></strong></label>
			<input class="text" id="telefon" type="text" name="Telefon" maxlength="40" value="" onfocus="clearField(this)" />
			<br class="clr" />
			</fieldset>

			<fieldset>
			<label class="text" for="plz"><strong title="Pflichtfeld"><?php print "PLZ"; ?></strong></label>
			<input class="text" id="plz" type="text" name="PLZ" maxlength="40" value="" onfocus="clearField(this)" />
			<br class="clr" />
			</fieldset>

			<fieldset>
			<label class="text" for="ort"><strong title="Pflichtfeld"><?php print "Ort"; ?></strong></label>
			<input class="text" id="ort" type="text" name="Ort" maxlength="40" value="" onfocus="clearField(this)" />
			<br class="clr" />
			</fieldset>

			<fieldset>
			<label class="text" for="firma"><strong><?php print "Firma"; ?></strong></label>
			<input class="text" id="firma" type="text" name="Firma" maxlength="40" value="" onfocus="clearField(this)" />
			<br class="clr" />
			</fieldset>

			<fieldset>
			<label class="text" for="dienststelle"><strong><?php print "Dienststelle"; ?></strong></label>
			<input class="text" id="dienststelle" type="text" name="Dienststelle" maxlength="40" value="" onfocus="clearField(this)" />
			<br class="clr" />
			</fieldset>

			<fieldset>
			<label class="text" for="position"><strong><?php print "Position"; ?></strong></label>
			<input class="text" id="position" type="text" name="Position" maxlength="40" value="" onfocus="clearField(this)" />
			<br class="clr" />
			</fieldset>

			<fieldset class="textarea">
			<label class="textarea" for="beschreibung"><strong title="Pflichtfeld"><?php print "Bemerkungen"; ?></strong></label>
			<textarea id="beschreibung" cols="" rows="4" name="Beschreibung" onfocus="clearField(this)"></textarea>
			<br class="clr" />
			</fieldset>

			<p>
			Der Nutzer <?php print $DATA["Name"]; ?> (E-Mail: <?php print $DATA["EMail"]; ?>) beantragt ueber das Geoportal.SL die Freischaltung ihres Dienstes/Layers <?php print $_REQUEST["TITLE"]; ?> mit der ID <?php print $_REQUEST["ID"]; ?>.
			</p>
			...
			<input type="submit" value="<?php print "Absenden"; ?>" />
			</fieldset>

			</form>

			<?php
			$values='id='.$_REQUEST["ID"].'&type='.$_REQUEST["TYPE"];
			//$code=CodeParameter($values);
			$code=$values;
			print '<p><a onclick="return tou(\''.$code.'\')">Hinweise zur Nutzung des Dienstes</a></p>';
			} else {

			$Subject="Antragsstellung auf die Freigabe eines Dienstes im GeoPortal.SL";
			//$MailFrom="info@q4u.de";

			$MailFrom="geoportal.saarland@lkvk.saarland.de";
			$MailTo=$_REQUEST["TO"];
			...
			$MailText='Der Nutzer '.$_REQUEST["Name"].' (E-Mail: '.$_REQUEST["EMail"].') beantragt ueber das Geoportal.SL
            ...
            $mailresult = $adm->sendEmail(MAILADMIN, MAILADMINNAME, $MailTo, $MailTo, $Subject, utf8_decode($MailText) );
				//if(mail($MailTo,$Subject,utf8_decode($MailText),"From: ".$MailFrom,"-f".$MailFrom)) {
			if ($mailresult) { 
				print "<p>Wir werden Ihre Anfrage in Kürze bearbeiten.</p>";
			} else {
				print "<p>Bei der Übermittlung Ihrer Anfrage ist ein Fehler aufgetreten. Bitte versuchen Sie es später noch einmal</p>";
			}
			}
